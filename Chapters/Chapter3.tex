...
This is important as it sheds light on why \ac{ANN}s require much more data to perform tasks which would take a human baby very little time to master. 

The cognitive development of infants can be simulated through transfer learning. In \cite{vinyals2015show} it was demonstrated that by pretraining the image feature extraction network on an image classification task and the language model on a word prediction task, the combined network was able to produce correct captions for images with only a small amount of training direct training. 
