...
The addition of more variety of colours to the dataset has had a negative effect on description accuracy in the Image Only testing condition, with a small number of images being mislabelled. The \ac{MAE} initialised with weights from expeirment 2 performed the best with a description generation accuracy of 99.53\% (compared to 99.99\% for the top performing model in experiment 3, \textcolor{red}{133 incorrectly described images versus only 1 incorrectly described in experiment 3, note that experiment 3 uses less data than experiment 4 hence the large difference between the number of incorrectly descirbed images)} \textcolor{red}{In the worst case, when training from scratch, the description accuracy in the Image Only testing condition drops to 96.10\% (1106 incorrectly described images).}
